  ...
  return (
    <section className="glass-panel rounded-[1.75rem] p-5 sm:rounded-[2rem] sm:p-6">
      <div className="mb-5">
        ...
        <p className="mt-2 text-sm leading-6 text-muted">Add usernames separated by commas or new lines. Everyone listed here needs an account already.</p>
      </div>
      ...
      <form action={formAction} className="space-y-4">
        ...
        <label className="block space-y-2 text-sm font-medium">
          <span>Currency</span>
          <select name="currency" defaultValue="SEK" className="w-full rounded-2xl border border-line bg-white/80 px-4 py-3 outline-none transition focus:border-accent">
            {SUPPORTED_CURRENCIES.map((code) => (
              <option key={code} value={code}>{code}</option>
            ))}
          </select>
          {state.fieldErrors?.currency ? <p className="text-sm text-danger">{state.fieldErrors.currency[0]}</p> : null}
        </label>

        <label className="block space-y-2 text-sm font-medium">
          <span>Member usernames</span>
          <textarea name="memberUsernames" rows={4} placeholder={"oscar\nlinda"} className="w-full rounded-2xl border border-line bg-white/80 px-4 py-3 outline-none transition focus:border-accent" />
          {state.fieldErrors?.memberUsernames ? <p className="text-sm text-danger">{state.fieldErrors.memberUsernames[0]}</p> : null}
        </label>
